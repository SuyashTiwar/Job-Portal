...
export const postJob=catchAsyncError(async(req,res,next)=>{
    const {role}=req.user;// destructuring is done
    if(role==='Job Seeker'){
    ...
    const {
      ...
      res.status(200).json({ // display mess.
        success: true,
...
//only employee can see their job only...
...
export const updateJob=catchAsyncError(async(req,res,next)=>{
  ...
  const {id}=req.params;// yai params hota hai param nahi--> (s) is important to be placed in params.
